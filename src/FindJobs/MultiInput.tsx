...
const MultiInput=(props:any)=> {
  ...
  const options = data.filter((item) => item.toLowerCase().includes(search.toLowerCase())) .map((item) => (
    ...
    <div className='text-bright-sun-400 p-1 bg-mine-shaft-900 rounded-full mr-2'><props.icon/></div>
          ...
          <Pill.Group>
            {value.length > 0 ? (
              ...
              <Input.Placeholder style={{ color: '#D3D3D3', fontSize: '16px' }}  >{props.title}</Input.Placeholder>
            )}
